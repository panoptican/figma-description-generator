...
PR #2 is merged at `dcc3af8ed102ce02bf5115d710895922087525a9`. The publisher-managed generation service, usage limits, and UI are the working baseline; merging them does not publish the plugin or establish a working paid checkout.
...
On September 10, 2026, Jason confirmed that Figma monetization was enabled and chose to continue with native payments. This investigation is closed without implementing third-party payments. Continue development on `main`; the external-payment research below is retained only as a fallback if that decision changes. Republish approval and a live paid checkout remain separate verification steps.
...
- These sources were checked September 10, 2026. Jason subsequently confirmed that monetization was enabled. No message to Figma was sent for this investigation.
...
This investigation produced notes only. Production still uses the Figma payment identity and checkout path from PR #2.
